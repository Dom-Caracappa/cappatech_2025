...
const ContactSection: React.FC = () => {
    ...
    // 1) Use environment var or default for the server URL
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            // 2) Instead of "/api/contact", call the full server URL
            const response = await fetch(`${BACKEND_URL}/api/contactSubmit`, {
                method: "POST",
    ...
    return (
        ...
        >
            <div className="bg-gray-800 shadow-lg rounded-2xl p-8 w-full max-w-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
                ...
                {message && (
                    <p
                        className={`text-center font-semibold mt-4 ${status === "success" ? "text-green-500" : "text-red-500"
                            }`}
                    >
                        {message}
                    </p>
