...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, planTitle }) => {
    // 1) Use an environment variable or default to "http://localhost:5042"
    //    This matches Step 5's approach, ensuring we call the same unified backend.
    const BACKEND_URL = import.meta.env.PUBLIC_SERVER_URL || "http://localhost:5042";
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Debugging logs
        console.log("🔄 Submitting pricing form to:", BACKEND_URL);
        console.log("Form data:", data);

        try {
            const response = await fetch(`${BACKEND_URL}/api/pricingSubmit`, {
                method: "POST",
...
export default function Pricing(): JSX.Element {
    ...
    return (
        <section
            id="pricing"
            className="w-full mb-16 mt- border-orange-500 rounded-md max-w-screen-xl px-6 py-6 md:py-6 bg-gray-900 text-white shadow-2xl shadow-black"
        >
            <h2 className="text-center text-2xl font-bold mb-8 text-orange-500">Our Pricing</h2>
